...
let list pr = list' "[" "; " "]" pr
  ...
  let rec iformula = function
    ...
    | ListF(Ast.T.Conj, l) -> sprintf  "(%s)" (Print.list' "" "^" "" iformula l) 
    | ListF(Ast.T.Disj, l) -> sprintf  "(%s)" (Print.list' "" " v " "" iformula l) 
  ...
  and program = function
    ...
    | ListP(Ast.T.Seq, l) -> sprintf "%s" (Print.list' "" "; " "" program l)
    | ListP(Ast.T.U, l) -> sprintf "%s" (Print.list' "" " U " "" program l)
  ...
  let rec arbre = function
    ...
    | Noeud(st, l, op, _) -> sprintf "%s%s(%s)"  ((function And -> "And" | Or -> "Or" | Not -> "Not") op)  ((function Ni -> "" | Proven true -> "(T)" | Proven false -> "(F)")st) (Print.list' "" ", " "" arbre l)
    | Feuille(Formu(v, f)) -> sprintf "%s|=%s" (set' "{" "," "}" v) (iformula f)
   | Feuille(Direct(b)) -> ((function true -> "T" | false -> "F") b)

  let rec arbreK = function
    (*| Noeud(_, l, op, _) -> sprintf "%s(%s)"  ((function And -> "And" | Or -> "Or " | Not -> "Not") op)  (Print.list' "" ", " "" arbre l)*)
    | Noeud(st, l, op, se) -> sprintf "[%d]%s%s(%s)"  se.n ((function And -> "And" | Or -> "Or" | Not -> "Not") op)  ((function Ni -> "" | Proven true -> "(T)" | Proven false -> "(F)")st) (Print.list' "" ", " "" arbreK l)
    | Feuille(Formu(v, f)) -> sprintf "%s|=%s" (set' "{" "," "}" v) (iformula f)
   | Feuille(Direct(b)) -> ((function true -> "T" | false -> "F") b)
...
let rec countP p = match p with
  | Formula.Assign(a, psi) -> CSet.add a (countF psi)
  | Formula.ListP(_, l) -> List.fold_right CSet.union (List.map countP l) CSet.empty
  | Formula.Test psi -> countF psi
  | Formula.Kleene p -> countP p
  | Formula.Converse _ -> raise (Not_implem "Converse")
and countF f = match f with
  | Formula.CallF(_, a) ->CSet.singleton a
  | Formula.Base _ -> CSet.empty
  | Formula.ListF(_, l) -> List.fold_right CSet.union(List.map countF l) CSet.empty
  | Formula.Modal(_, p, f) -> CSet.union (countP p) (countF f)
...
let rec formToIForm (f:Formula.formula) : iformula = match f with
  | Formula.CallF(b, a) -> let _= if false then eprintf "---> %s %b\n" (Circuit.Print.callable a) b in 
      let s = Circuit.Print.callable a in CallF(b, s)
  | Formula.Base b -> Base b
  | Formula.ListF(op, l) -> ListF(op, List.map formToIForm l)
  | Formula.Modal(b, p, f) -> let fi = formToIForm f in
    match p with
    | Formula.ListP(Ast.T.Seq, xt) ->  (* this isn't working properly obviously TODO *)
        ( match xt with
        | [] -> formToIForm f (* skip.. = <;> *)
        | [h] -> Modal(b, progToIProg h, fi)
        | h::t -> Modal(b, progToIProg h, formToIForm (Formula.Modal(b, ListP(Ast.T.Seq, t), f)))
      )
    | _ -> let _=eprintf"still\n" in Modal(b, progToIProg p, fi)

and progToIProg (p:Formula.program) : iprogram = match p with
  | Formula.Assign(a, psi) -> Assign(Circuit.Print.callable a, formToIForm psi)
  | Formula.ListP(op, l) -> ListP(op, List.map progToIProg l) (*should convert Seq close to above, but not really necessary as we won't use this function directly *)
  | Formula.Test psi -> Test (formToIForm psi)
  | Formula.Kleene p -> LE(twopow (acountP p) , progToIProg p)
  | Formula.Converse _ -> raise (Not_implem "Converse")
...
let splitIF (v:SS.t) (f:iformula) : arbre = match f with
  ...
  | ListF(Ast.T.Conj, l) -> Noeud(Ni, List.map (toFormu v) l, And, statInit (List.length l))
  | ListF(Ast.T.Disj, l) -> Noeud(Ni, List.map (toFormu v) l, Or, statInit (List.length l))
  | Modal(b, p, f) -> match p with
    | Assign(s, fa) -> let vs = SS.add s v and vns = SS.remove s v in
    ...
    | ListP(Ast.T.Seq, l) -> (match l with 
        ...
        | h::t -> Feuille(Formu(v, Modal(b, h, Modal(b, ListP(Ast.T.Seq, t), f))))(*<p;q>-> <p><q>*)
    )
    | ListP(Ast.T.U, l) -> Noeud(Ni, List.map (ptoFormu v f b) l, bDiff b,statInit (List.length l))
    | LE(n, p) -> match n with
