...
let selectL n l op = if List.length l = 1 then List.hd l, 0 else
...
let rec selecti n id path = 
  ...
  | Noeud(_, op, l, _) -> let snode, num = (selectL n l op) in match feuille snode with
...
let updateNoeud dagnoeud (resultat:bool) id = let nnoeud = (match dagnoeud with
  Proven _ as p -> p
        | Noeud(_, op, l, stat) -> Noeud(true, op, l, {n=stat.n+1 ; nwin=stat.nwin + (if resultat then 1 else 0)}) 
