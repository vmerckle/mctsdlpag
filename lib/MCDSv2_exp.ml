...
let solve old_f ~n ~quicksolver ~constant ~playout =  
  let _ = nplayout := playout in
  let ob, iter = solve_new ~quicksolver ~n ~c:constant (Formula.formula_retread old_f)
